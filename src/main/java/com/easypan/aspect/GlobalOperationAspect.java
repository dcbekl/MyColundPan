...
public class GlobalOperationAspect {
    ...
    public void interceptorDo(JoinPoint point) throws BusinessException {
        try {
            Object target = point.getTarget();    // 获得目标对象
            Object[] arguments = point.getArgs(); // 获得目标参数
            String methodName = point.getSignature().getName(); // 获得目标函数名
            Class<?>[] parameterTypes = ((MethodSignature) point.getSignature()).getMethod().getParameterTypes();
            Method method = target.getClass().getMethod(methodName, parameterTypes);
            GlobalInterceptor interceptor = method.getAnnotation(GlobalInterceptor.class); // 获取方法上的GlobalInterceptor 注解
            if (null == interceptor) {
                return;
            }
            /**
